...
class TestOmeZarrMetadata(unittest.TestCase):
    ...
    def test_get_pyramid_metadata(self):
        ...
        self.assertEqual(result["metadata"]["method"], "tensorstore.downsample")
        self.assertEqual(result["metadata"]["description"], "Downscaling tensorstore downsample")
    ...
    def test_build_ome_minimal(self):
        ...
        self.assertEqual(result["channels"][0]["label"], "Channel:test_image:0")
    ...
    def test_build_ome_with_channel_names(self):
        ...
        result = _build_ome(data_shape, image_name, channel_names=channel_names)
    ...
    def test_build_ome_with_colors(self):
        ...
        result = _build_ome(data_shape, image_name, channel_colors=channel_colors)
    ...
    def test_build_ome_with_minmax(self):
        ...
        result = _build_ome(data_shape, image_name, channel_minmax=channel_minmax)
    ...
    def test_validate_omero_metadata_missing_window_key(self):
        ...
        omero_metadata = {
            "channels": [
                {
                    "color": "ff0000",
                    "window": {"min": 0.0, "max": 255.0, "start": 0.0},  # Missing 'end'
                }
            ]
        }
    ...
    @patch("aind_exaspim_data_transformation.compress.omezarr_metadata._build_ome")
    @patch("aind_exaspim_data_transformation.compress.omezarr_metadata._compute_scales")
    @patch("aind_exaspim_data_transformation.compress.omezarr_metadata._downscale_origin")
    @patch("aind_exaspim_data_transformation.compress.omezarr_metadata.add_multiscales_metadata")
    def test_write_ome_ngff_metadata(
        self, mock_add_multiscales, mock_downscale, mock_compute, mock_build_ome
    ):
        """Test write_ome_ngff_metadata integration"""
        # Mock return values
        mock_build_ome.return_value = {"channels": []}
        mock_compute.return_value = ([{"type": "scale"}], [{"chunks": (64, 64, 64)}])
