...
class TestTranslatePyramidLevels(unittest.TestCase):
    ...
    @patch("aind_exaspim_data_transformation.compress.imaris_to_zarr.Path")
    def test_translate_pyramid_with_additional_levels_calls_downsample(
        self,
        mock_path_cls,
        mock_reader_cls,
        mock_ts_open,
        ...
        """When translate_pyramid_levels=True and additional_levels>0,
        create_downsample_levels must be invoked once after the
        translate loop, with start_scale = (n_lvls - 1), n_levels =
        additional_levels + 1, and base_shape matching the last
        translated level. Metadata must advertise translated + extra
        levels combined."""
        from aind_exaspim_data_transformation.compress.imaris_to_zarr import (
            imaris_to_zarr_distributed,
        )
        ...
        # Mock ImarisReader with 3 translated levels.
        mock_reader = MagicMock()
        mock_reader.get_voxel_size.return_value = ([1.0, 0.5, 0.5], b"um")
        mock_reader.get_metadata_shape.return_value = (128, 256, 512)
        mock_reader.get_true_shape_for_level.side_effect = lambda lvl: {
        ...
        mock_reader.get_dtype.return_value = np.dtype("uint16")
        mock_reader.get_chunks.return_value = (32, 64, 128)
        mock_reader_cls.return_value.__enter__ = Mock(
            return_value=mock_reader
        )
        mock_reader_cls.return_value.__exit__ = Mock(return_value=False)
        ...
        mock_output_path = MagicMock()
        mock_output_path.__truediv__ = Mock(return_value=mock_output_path)
        mock_output_path.__str__ = Mock(
            return_value="/output/test.ome.zarr"
        )
        mock_path_cls.return_value = mock_output_path
        ...
        # create_downsample_levels called once for the extra levels.
        mock_downsample.assert_called_once()
        kwargs = mock_downsample.call_args.kwargs
        # n_levels in the chain = 1 base (last translated) + 3 new = 4.
        self.assertEqual(kwargs["n_levels"], 4)
        # start_scale points at the last translated level (index n_lvls-1).
        self.assertEqual(kwargs["start_scale"], 2)
        # base_shape is the 5D shape of the last translated level.
        self.assertEqual(kwargs["base_shape"], (1, 1, 32, 64, 128))
        # downsample knobs reused from existing settings.
        self.assertEqual(kwargs["downsample_factor"], (2, 2, 2))
        self.assertEqual(kwargs["downsample_mode"], "mean")

        # OME metadata advertises translated + computed levels.
        ome_kwargs = mock_write_ome.call_args.kwargs
        self.assertEqual(ome_kwargs["n_lvls"], 6)
    ...
    @patch("aind_exaspim_data_transformation.compress.imaris_to_zarr.Path")
    def test_translate_pyramid_additional_levels_skipped_by_non_coordinator(
        self,
        mock_path_cls,
        mock_reader_cls,
        mock_ts_open,
        ...
        """Non-coordinator workers (partition_to_process != 0 and no
        dask_client) must NOT compute extra levels even when
        additional_levels > 0; only worker 0 does the work."""
        from aind_exaspim_data_transformation.compress.imaris_to_zarr import (
            imaris_to_zarr_distributed,
        )
        ...
        mock_downsample.assert_not_called()
        # Non-coordinator workers also skip metadata write.
